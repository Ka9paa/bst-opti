...
import { applyOptimization, isElectronApp } from '../utils/electronOptimizations';
...
export function GameOptimizer({ game, packageId, onBack }: GameOptimizerProps) {
  ...
  const handleConfirmDownload = async () => {
    ...
    if (isElectronApp()) {
      // Desktop app - use Electron API for better UX
      const result = await applyOptimization(game, packageId, scripts.windowsScript);
      
      if (result.success) {
        toast.success('Optimization script created!', {
          description: 'Script saved to Documents/OPTIAXIRA_Optimizations'
        });
      } else {
        toast.error('Failed to save optimization script', {
          description: result.error
        });
      }
    } else {
      // Web version - fallback to downloads
